...
def get_dataset(img_path, bbox_path, is_multi=False, resize=64):
    ...
    for img_name, bbox_name in zip(img_names, bbox_names):
        datasets.append(DatasetGen(img_name, bbox_name, is_multi, resize))
    
    return ConcatDataset(datasets)


def get_loader(img_path, bbox_path, sample_mode, is_multi=False, resize=64, batch_size=1, sample_size=800, num_workers=4):
    assert sample_mode in ['all', 'sampled'], f'Invalid sample mode, got {sample_mode}.'
    
    dataset = get_dataset(img_path, bbox_path, is_multi = is_multi, resize = resize)
